  ...
  fetch(url)
    ...
    .then(function (response) {
      ...
      var newURL =
        ...
        "&lon=" +
        lon +
        "&appid=05477151a72eb8675c1e912165d66a19";
  ...
  fetch(newUrl)
    ...
    .then(function (response) {
      ...
      var authorNameElement =
        "Authors Name: " + response.docs[0].author_name[0];
      ...
      function addInfoBtn() {
        ...
        newBookText = document.createTextNode(
          "Find More About: " + searchInput.value + " Here!"
        );
...
if (pickedMember == "Cynthia") {
  ...
  const randomDrinkForCynthia = Math.floor(
    Math.random() * CynthiasDrinks.length
  );
  ...
  recommendations.innerHTML =
    "Cynthia recommends to drink a " +
    pickedDrinkForCynthia +
    " and a " +
    pickedFoodForCynthia +
    " to eat with this book";
}
if (pickedMember == "Bryan") {
  const BryansDrinks = [
    "Iced Coffee with TWO sugars & THREE Creamers",
    "Cold Brew",
    "'Frappacino Icey Type Thing'",
    "Water",
    "'Nothing'",
  ];
  const BryansFoods = [
    "'Spinach, Feta Wrap Thingy'",
    "Bagel with Cream Cheese",
    "Cake Pops",
    "Ham & Swiss Croissant",
    "Bowl Of Cereal",
